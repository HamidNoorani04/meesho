...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    // Add small delay to avoid PayU rate limit
    sleep(2);
    
    // Generate highly unique transaction ID
    $txnid = 'MEESHO' . time() . rand(1000, 9999) . uniqid();
    $amount = $total;
    $productinfo = 'Meesho Order - ' . date('Y-m-d H:i:s');
    $firstname = $_SESSION['address']['full_name'];
    $email = 'customer' . time() . '@meesho.com'; // Unique email
    $phone = $_SESSION['address']['mobile'];
